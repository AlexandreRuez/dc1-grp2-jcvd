...
function getEntity(string $table,int $id): array {
    ...
    $query = "SELECT * FROM " . $table . " categorie WHERE id=:id";
...
function getAllPhotosByTag(int $id): array {
    ...
    $query = "SELECT
                ...
                photo.nb_likes
            FROM photo
            INNER JOIN photo_has_tag ON photo.id=photo_has_tag.photo_id
...
function getAllcommentaires(int $id): array {
    global $connection;

    $query = "SELECT * FROM commentaire WHERE photo_id = ".$id." ORDER BY date_creation;";
